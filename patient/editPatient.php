...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $conn->prepare("UPDATE patient SET pname = ?, pemail = ?, pdob = ?, pgender = ? WHERE pid = ?");
        $stmt->bind_param('ssssi', $name, $email, $age, $gender, $pid);
    ...
    $stmt = $conn->prepare("SELECT pname, pemail, pdob, pgender FROM patient WHERE pid = ?");
...
<head>
    ...
    <title>Edit Profile || Appoint Me</title>
